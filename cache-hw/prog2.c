...
volatile int global_array[1048568] __attribute__((aligned(128)));
...
int main() {
    ...
    const int SKIP1 = 324288;
    const int SKIP2 = 262146;
    const int SKIP3 = 131073;       
...
#pragma clang loop interleave(disable)
    ...
    for (int i = 0; i < MAX; i+=SKIP1) {
        global_array[i] = (i + SKIP2) % MAX;       
        global_array[(i + SKIP2) % MAX] = (i + SKIP3) % MAX; 
        global_array[(i + SKIP3) % MAX] = (i + SKIP1) % MAX;  
    }
